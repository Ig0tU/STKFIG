...
export class DoodleGame {
  ...
  aiThink() {
    ...
    if (dist > 170) {
      ai.vel.x = Math.sign(dx) * ai.walk * 0.82;
      if (ai.grounded && Math.random() < 0.015) {
        ai.vel.y = ai.jumpF;
        ai.jumps--;
      }
    } else if (dist < 55 && Math.random() < 0.05) {
      if (Math.random() < 0.5) {
        ai.attack("GRAB", this.audio);
      } else {
        ai.vel.x = -Math.sign(dx) * ai.walk;
      }
    } else {
      if (this.p1.state === "JAB" || this.p1.state === "KICK" || this.p1.state === "UPPER") {
        if (Math.random() < 0.45) this.aiBlockT = 16;
      }
      ...
      } else if (ai.ink >= 100 && roll < 0.11) {
        ai.attack("SPECIAL", this.audio);
      } else if (ai.ink >= 50 && roll < 0.16) {
  ...
  resolve(a: Fighter, b: Fighter) {
    ...
    if (d < hb.r + 20) {
      ...
      if (didParry) {
        ...
        this.splatter(hb.x, hb.y, "#2f4a68", 10);
        return;
      }
